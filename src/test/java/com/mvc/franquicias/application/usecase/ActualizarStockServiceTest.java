...
class ActualizarStockServiceTest {

    @Mock
    private FranquiciaRepositoryPort franquiciaRepositoryPort;

    private ActualizarStockService service;

    @BeforeEach
    void setUp() {
        service = new ActualizarStockService(franquiciaRepositoryPort);
    }

    @Test
    void deberiaActualizarStockDeProductoExistente() {
        ...
        StepVerifier.create(service.actualizarStock("f1", "s1", "p1", 25))
                .assertNext(actualizada -> {
                    Producto productoActualizado = actualizada.sucursales().get(0).productos().get(0);
                    assertThat(productoActualizado.stock()).isEqualTo(25);
                })
                .verifyComplete();
    }
